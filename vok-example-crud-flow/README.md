...
* Linking to a database. VaadinOnKotlin uses [vok-orm](https://github.com/mvysny/vok-orm) for simple O/R mapping when accessing the database.
  ...
  To link to the database, we configure Hikari database connection pooler in [Bootstrap.kt](src/main/kotlin/com/github/vok/example/crudflow/Bootstrap.kt).
...
* Preparing the database: simply run Flyway migration every time before the app is started, to make sure that the app has newest database ready.
  ...
  Please see [Bootstrap.kt](src/main/kotlin/com/github/vok/example/crudflow/Bootstrap.kt)
...
* Accessing the database: just create your pojo beans [(example Person)](src/main/kotlin/com/github/vok/example/crudflow/person/Person.kt)
...
* Serving the data via REST: add [vok-rest](../vok-rest) to your project, see [build.gradle](build.gradle). Then, declare REST Application to bind
  ...
  [Bootstrap.kt](src/main/kotlin/com/github/vok/example/crudflow/Bootstrap.kt)
  the `@ApplicationPath("/rest")` stanza. After that, just define your REST-accessing classes, for example
  [PersonRest](src/main/kotlin/com/github/vok/example/crudflow/PersonRest.kt)
* Creating the UI: there are lots of great Vaadin tutorials, in general you declare your view and populate it with components. See
  [PersonListView](src/main/kotlin/com/github/vok/example/crudflow/person/PersonListView.kt)
* Create Update Delete (CRUD): no Scaffolding-like UI generator for now, but you can see the
  [crud example](src/main/kotlin/com/github/vok/example/crudflow/person) on how to write the CRUD UI yourself very easily.
...
* Testing: uses the [Karibu-Testing](https://github.com/mvysny/karibu-testing) framework; please find the example test at [PersonListViewTest.kt](src/test/kotlin/com/github/vok/example/crudflow/person/PersonListViewTest.kt).
