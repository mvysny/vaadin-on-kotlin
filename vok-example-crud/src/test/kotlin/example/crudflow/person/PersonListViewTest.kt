...
class PersonListViewTest : AbstractAppTest() {
    ...
    @Test fun `grid is refreshed when data is generated`() {
        ...
        val grid = _get<Grid<*>>()
        grid.expectRows(86)
        val first = Persons.findAll().first { it.name == "generated0" }
    ...
    @Test fun `edit one person`() {
        Person {
            name = "Leto Atreides"; age = 45; dateOfBirth = LocalDate.of(1980, 5, 1)
            maritalStatus = MaritalStatus.Single; alive = false; created = Instant.now()
        }.create()
        ...
        expectList("Duke Leto Atreides") { Persons.findAll().map { it.name } }
    }
