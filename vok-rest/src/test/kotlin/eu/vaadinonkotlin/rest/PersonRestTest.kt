...
class PersonRestTest : AbstractJavalinTest() {
    ...
    @Nested inner class `write ops return 501 until Gson-Entity adapter is wired up` {
        @Test fun create() {
            val dto = PersonDto(personName = "X", age = 30, maritalStatus = MaritalStatus.Single, alive = true, created = Instant.now())
            val ex = assertThrows<HttpResponseException> { crud.create(dto) }
            expect(501) { ex.statusCode }
        }
        @Test fun update() {
            val ex = assertThrows<HttpResponseException> {
                crud.update("1", PersonDto(personName = "X", age = 30, maritalStatus = MaritalStatus.Single, alive = true, created = Instant.now()))
            }
            expect(501) { ex.statusCode }
        }
        @Test fun delete() {
            val ex = assertThrows<HttpResponseException> { crud.delete("1") }
            expect(501) { ex.statusCode }
        }
    }
}
